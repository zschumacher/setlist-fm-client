...
class TestBuildHeaders:
    def test_when_accept_is_none_value_error_raises(self):
        with pytest.raises(ValueError):
            _build_headers(None, None, SetlistFmClient())

    @pytest.mark.parametrize("headers", [{"hello": "world"}, None])
    def test_different_header_kwargs(self, headers):
        headers = _build_headers(Accept.json, None, SetlistFmClient(), headers=headers)
    ...
    def test_passed_api_key(self, client, monkeypatch):
        ...
        headers = _build_headers(Accept.json, "super-secret-key", client())
    ...
    def test_instance_api_key(self, client, monkeypatch):
        ...
        headers = _build_headers(Accept.json, None, client())
        assert headers["x-api-key"] == "super-secret-key"
    ...
    def test_accept_header_is_set(self, accept):
        headers = _build_headers(accept.value, "super-secret-key", SetlistFmClient())
